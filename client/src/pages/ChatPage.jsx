...
const ChatPage = () => {
  ...
  useEffect(() => {
    ...
    // Emit setup event upon connection
    ...
    socketRef.current.on('message', (receivedMessage) => {
      ...
      if (receivedMessage.conversationId === activeChat?.id) {
        ...
        setMessages((prevMessages) => {
          ...
          if (isDuplicate) {
            // Update existing message if it was optimistic
            return prevMessages.map((msg) =>
              msg.id === receivedMessage.tempId
                ? { ...formatReceivedMessage(receivedMessage, user?._id), sender: msg.sender }
                : msg
            );
          }
          // Add new message
          const formattedMessage = formatReceivedMessage(receivedMessage, user?._id);
  ...
  }, [isAuthenticated, user?._id, activeChat?.id]);
   ...
   const handleAddUserConfirm = useCallback(async (conversationId, userIdToAdd) => {
    ...
    const handleSendTextMessage = useCallback(async () => {
        ...
        try {
          ...
          // Optionally keep HTTP API call for consistency (or remove if backend handles via Socket.IO)
