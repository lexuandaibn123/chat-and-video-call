...
import {
    ...
    deleteMessageApi, // Added import
    editMessageApi,   // Added import
...
const ChatPage = () => {
  ...
  } = useSocketIO(activeChat?.id, user?._id);
  ...
  // EFFECT 4: Sync Socket.IO messages to state
  useEffect(() => {
    if (socketMessages && activeChat?.id) {
  ...
  }, [socketMessages, activeChat, setIsMobileChatActive]);

  // EFFECT 5: Handle Socket.IO errors
  useEffect(() => {
    if (socketError) {
  ...
  const handleRemoveUser = useCallback(
    async (conversationId, userIdToRemove) => {
      ...
      if (
        window.confirm(`Are you sure you want to remove ${memberToRemove.fullName || userIdToRemove} from the group?`)
      ) {
  ...
  const handleChangeLeader = useCallback(
    async (conversationId, newLeaderId) => {
      ...
      if (
        window.confirm(`Are you sure you want to make ${newLeaderMember.fullName || newLeaderId} the new leader?`)
      ) {
        await performSettingsAction(
  ...
  const handleSendTextMessage = useCallback(
    async () => {
      ...
      if (
        ...
        console.warn(
          "Cannot send text message: Invalid state (no chat, no user, sending, editing, empty, or socket disconnected)."
        );
        setActionError("Cannot send message. Please check connection or try again.");
        return;
      }
      ...
      try {
        ...
        await sendTextMessage(messagePayload, tempId);
        setConversations((prevConversations) =>
          updateConversationsListLatestMessage(prevConversations, activeChat.id, {
            ...messagePayload,
            _id: tempId,
          })
        );
  ...
  const handleSendFile = useCallback(
    async (file) => {
      ...
      if (
        ...
        console.warn(
          "Cannot send file: Invalid state (no chat, no user, sending/editing in progress, no file, or socket disconnected)."
        );
        setActionError("Cannot send file. Please check connection or try again.");
        return;
      }
      ...
      try {
        ...
        await sendFileMessage(messagePayload, tempId);

        setConversations((prevConversations) => {
          return updateConversationsListLatestMessage(prevConversations, activeChat.id, {
            ...messagePayload,
            _id: tempId,
          });
        });
  ...
  const showInitialLoading =
    isAuthLoading || (isAuthenticated && user?._id && isLoadingConversations && !activeChat && !error);
  const showAuthError = !isAuthLoading && !isAuthenticated;
  const showGeneralError =
    error && !isLoadingConversations && !isLoadingMessages && !isPerformingAction && !sendingMessage && !actionError && isAuthenticated;
