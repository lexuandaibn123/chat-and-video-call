...
export const useHandlers = ({
    ...
    async (apiCall, successMessage, updateStateFunc = null) => {
      ...
      try {
        ...
        if (response && response.success === false) {
          console.error(
            `${successMessage} failed: API indicated failure.`,
            response
          );
          const errorMessage =
        ...
        setActionError(
          err.message ||
            `An API error occurred during ${successMessage.toLowerCase()}.`
        );
      } finally {
  ...
  const handleConversationClick = useCallback(
    async (type, id) => {
      console.log("handleConversationClick called:", {
        type,
        id,
        user,
  ...
  const handleSendTextMessage = useCallback(async () => {
    ...
    if (
      ...
      console.warn(
        "Cannot send text message: Invalid state or socket not connected"
      );
      setActionError(
        "Cannot send message: Socket not connected or invalid state."
      );
      return;
    }
    ...
    try {
      ...
      setMessages((prevMessages) =>
        prevMessages.map((msg) =>
          msg.id === tempId
            ? { ...newMessageOptimistic, status: "failed" }
            : msg
        )
      );
    } finally {
  ...
  const handleUploadBeforeBegin = useCallback(
    (files) => {
      ...
      if (
        ...
        console.warn(
          "Cannot upload file(s): Invalid state or no files selected."
        );
  ...
  const handleDeleteMessage = useCallback(
    async (messageId) => {
      ...
      if (!messageId || !activeChat?.id || !currentUserId) {
        console.warn(
          "Cannot delete message: Invalid parameters or no active chat/user."
        );
      ...
      if (!messageToDelete) {
        console.warn(
          `Cannot delete message ${messageId}: Message not found in state.`
        );
      ...
      if (
        ...
        console.warn(
          `Cannot delete message ${messageId}: Message status is '${messageToDelete.status}'.`
        );
      ...
      if (messageId === editingMessageId) {
        console.warn(
          `Cannot delete message ${messageId}: Message is currently being edited.`
        );
  ...
  const handleInitiateEditMessage = useCallback(
    async (messageId, currentText) => {
      ...
      if (!messageId || !activeChat?.id || !currentUserId) {
        console.warn(
          "Cannot initiate edit message: Invalid parameters or no active chat/user."
        );
        return;
      }
      ...
      if (!messageToEdit) {
        console.warn(
          `Cannot initiate edit message ${messageId}: Message not found in state.`
        );
      ...
      if (
        ...
        console.warn(
          `Cannot initiate edit message ${messageId}: Not the sender or not a text message.`
        );
      ...
      if (messageToEdit.status !== "sent") {
        console.warn(
          `Cannot initiate edit message ${messageId}: Message status is '${messageToEdit.status}'.`
        );
      ...
      if (sendingMessage || editingMessageId !== null) {
        console.warn(
          `Cannot initiate edit message ${messageId}: Another action is in progress.`
        );
        setActionError(
          editingMessageId !== null
      ...
      console.log(
        `Initiating edit for message ${messageId} with text: "${currentText}"`
      );
    },
    [
      activeChat?.id,
